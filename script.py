...
def main():
    ...
    for item in earthquakes[:20]:
        ...
        # The markers in the map will be the same size as the magnitude plus 5.
        magnitudes.append(item[0] + 5)

    # Save Markdown to file.
    open("./README.md", "w", encoding="utf-8").write(markdown)

    generate_map(longitudes, latitudes, magnitudes)


def get_data():
    """Requests the earthquake data from the USGS API.

    Returns
    -------
    dict
        The serialized API data.
...
def generate_map(longitudes, latitudes, magnitudes):
    """Generates a world map with the given longitudes, latitudes and magnitudes.
    ...
    fig.add_traces(go.Scattergeo(lon=longitudes, lat=latitudes,
                                 mode="markers", marker_color="gold", marker_size=magnitudes))
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                ...
                y=-0.0,
                yanchor="bottom",
                text=f"Source: USGS API | Last updated: {datetime.now():%m-%d-%Y @ %H:%M:%S} (UTC)",
                font_size=26
            ),
            dict(
                x=0.5,
